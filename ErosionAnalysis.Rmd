...
SoilHealthDB <- read.csv('../SoilHealthDB/data/SoilHealthDB_V2.csv', header = T)
...
# test results
SoilHealthDB %>% select (Texture, TextureGroup) %>% filter(TextureGroup == "NotAvailable") %>% distinct(Texture)
unique(SoilHealthDB[which((SoilHealthDB$Texture=="")),]$TextureGroup)

# SoilHealthDB <- SoilHealthDB[SoilHealthDB$Conservation_Type == "AFS",] 
erosion_meta <- read.csv('outputs/erosion_meta.csv')
# Subset
...
sub_runoff %>% select(Runoff_C) %>% order
...
siteInfor_erosion <- sub_erosion %>% 
  select(YearPublication, Latitude, Longitude) %>% 
...
```{r}
sub_erosion %>% select(StudyID) %>% unique
...
```{r}
SoilHealthDB %>% select(Erosion_T, StudyID, Conservation_Type) %>% 
  na.omit() %>% 
  select(StudyID, Conservation_Type) %>% 
...
```{r}
SoilHealthDB %>% select(Erosion_C, StudyID, TextureGroup) %>% 
  na.omit() %>% 
  select(Erosion_C, TextureGroup) %>% 
...
```{r}
SoilHealthDB %>% 
  select(Erosion_C, Erosion_T, Runoff_C, Runoff_T) %>% 
  na.omit() %>% 
...
cor(x = sub_shdb$Erosion_RR, y = sub_shdb$Runoff_RR, method = "pearson") %>% summary()
```
